...
<div class="min-h-screen flex items-center justify-center relative overflow-hidden">
    ...
    <div class="relative z-20 flex items-center justify-center w-full min-h-screen px-4">
        <div class="relative bg-white rounded-[48px] shadow-2xl w-full max-w-xl mx-auto flex flex-col items-center pt-20 pb-10 px-8">
            ...
            <h2 class="text-3xl font-bold text-black mb-8 text-center mt-2">Bienvenido de nuevo</h2>

            <form method="POST" action="{{ route('login') }}" class="w-full flex flex-col gap-5">
                @csrf

                <div>
                    <label for="email" class="block text-gray-700 font-semibold mb-1">Username *</label>
                    <input id="email" type="email" name="email" placeholder="Enter your Username" value="{{ old('email') }}" required autofocus autocomplete="username"
                ...
                <div class="flex items-center mb-2">
                    <input type="checkbox" name="remember" id="remember_me" class="form-checkbox text-blue-600">
                    <label for="remember_me" class="ml-2 text-gray-700 text-sm">Remember me</label>
                </div>

                <button type="submit" class="w-full bg-blue-700 text-white py-3 rounded-md font-semibold hover:bg-blue-800 transition text-lg mt-2">Acceso</button>
            </form>

            <div class="w-full flex flex-col items-center mt-8 gap-2">
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-gray-400 hover:text-blue-700 text-base">¿Se te olvidó tu contraseña?</a>
                @endif
                <a href="{{ route('register') }}" class="text-gray-400 hover:text-blue-700 text-base">¿No tienes una cuenta?</a>
            </div>
        </div>
    </div>
</div>
